...
class ListEvents extends Component<Props> {
  ...
  handleSubmit = (event: any) => {
    ...
    const {
      ...
      { dispatch } = this.props,
      postEventAction = postEvent({
        title: title.value,
        description: description.value,
        start_datetime: startDateTime.value,
        end_datetime: endDateTime.value,
        organisation: Number(organisation.value),
      });

    dispatch(postEventAction);
  };
            ...
            <Col>
              <div className="App-intro">
                ...
                {events.events.map(event => {
                  return (
                    <div key={event.id}>
                      {event.title} by {event.organisation.name}
                    </div>
                  );
                })}
              </div>
          ...
          <Row>
            <Col>
              ...
              <form onSubmit={this.handleSubmit}>
                ...
                <Button tag="button" color="success" size="small" type="submit">
                  Add event
                </Button>
              </form>
            </Col>
